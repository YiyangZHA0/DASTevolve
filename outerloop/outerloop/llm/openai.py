...
class EmptyOpenAIResponseError(RuntimeError):
    pass
...
class OpenAILLM(LLMInterface):
        ...
        if self.manual_mode:
            ...
            if not qdir:
                raise ValueError(
                    ...
                    "This should be injected by the OuterLoop controller."
                )
    ...
    async def _call_api(self, params: Dict[str, Any]) -> str:
        ...
        if not isinstance(content, str) or not content.strip():
            ...
            safe_finish_reason = finish_reason[:80].replace("\n", " ")
            raise EmptyOpenAIResponseError(
                "OpenAI-compatible API returned empty completion content "
                f"(finish_reason={safe_finish_reason!r})"
            )
        logger.debug(
            "OpenAI-compatible API completion received: model=%r finish_reason=%r",
            params.get("model"),
            finish_reason,
        )
        return content
